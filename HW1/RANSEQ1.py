...
if __name__ == "__main__":
	if len(sys.argv) != 7:
		print "USAGE: python RANSEQ1.py -i [INPUT_FILE] -n [Number_Of_Sequence] -o [OUTPUT_FILE]"
		exit(-1)
...
#np.random.choice()
#randomly generate required number of output sequences with fixed frequency


SD_Ade, SD_Thy, SD_Cyt, SD_Gua = [], [], [], []

for i in range( num ):
    list_to_choose = ['A', 'T', 'C', 'G']
    ran_seq = np.random.choice(list_to_choose, len(seq), p = [Ade, Thy, Cyt, Gua])
    ran_seq_string = ''.join(ran_seq)
    f.write("The %dth randomly generated sequence is: " % (i+1)) 
